	...
	client.on('message-new', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'nekonime':
					teks = body.slice(9)
					nekonime = await getBuffer(`https://arugaz.herokuapp.com/api/nekonime`, {method: 'get'})
					client.sendMessage(from, nekonime, image, {quoted: mek})
					break
				case 'waifu':
					teks = body.slice(6)
					waifu = await getBuffer(`https://arugaz.herokuapp.com/api/waifu`, {method: 'get'})
					client.sendMessage(from, waifu, image, {quoted: mek})
					break
				case 'loli':
					teks = body.slice(5)
					loli = await getBuffer(`https://arugaz.herokuapp.com/api/randomloli`, {method: 'get'})
				...
				case 'infoig':
					if (args.length < 1) return reply('Teks nya mana cuyy')
					teks = body.slice(8)
					anu = await fetchJson(`https://alfians-api.herokuapp.com/api/stalk?username=${teks}`, {method: 'get'})
					ig = await getBuffer(anu.Profile_pic)
					aabb = `*Biodata* ${anu.Biodata}*\n*Jumlah Followers* : ${anu.Jumlah_Followers}\n*Jumlah Following* : ${anu.Jumlah_Following}\n*Jumlah Post* : ${anu.Jumlah_Post}\n*Username* : ${anu.Username}`
					client.sendMessage(from, aabb, image, {quoted: mek, caption: `${aabb}`})
				case 'phub':
					if (args.length < 1) return reply('Teks nya mana cuyy')
					gh = body.slice(6)
					ph1 = gh.split(" ")[0];
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih cuyy'})
					break
				case 'memeindo':
