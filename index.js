		...
		try {
			...
			switch(command) {
				...
				case 'nsfwneko':
					teks = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/random/nsfwneko?apiKey=${apiKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					nsfw = await getBuffer(anu.result)
					client.sendMessage(from, nsfw, image, {quoted: mek, caption: '>///<'})
					break
				case 'quotes':
					teks = body.slice(8)
