	...
	client.on('message-new', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'bpink':
					teks = body.slice(7)
					tahta = await getBuffer(`https://api.vhtear.com/blackpinkicon?text=${teks}&apikey=${vkey}`, {method: 'get'})
					client.sendMessage(from, bpink, image, {quoted: mek})
					break
				case 'party':
					teks = body.slice(7)
					tahta = await getBuffer(`https://api.vhtear.com/partytext?text=${teks}&apikey=${vkey}`, {method: 'get'})
