		...
		try {
			...
			switch(command) {
				...
				case 'glitch':
					...
					break
				case 'resep':
					if (args.length < 1) return reply('Nama masakannya mana cuyy')
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/resepmasakan?query=${body.slice(7)}&apikey=${vkey}`)
					if (anu.error) return reply(anu.error)
					const rsp = `*Judul* : ${anu.result.title}\n*Deskripsi* : ${anu.result.desc}\n*Bahan* : ${anu.result.bahan}\n*Cara* : ${anu.result.cara}\n\n*Sumber* : ${anu.result.source}`
					rsep = await getBuffer(anu.image)
					client.sendMessage(from, rsep, image, {quoted: mek, caption: `${rsp}`})
					break
				case 'infoig':
